<template>
    <div class="container-task-data" v-if="haveData">  
        ...
        <div class="form-task-data">
            ...
            <div class="item-task-data">
                <p>Cambiar fase a:</p>
                 <select v-bind:class="{ 'user-border': user.rol=='user',' man-border': user.rol=='manager' }"  v-model="myTask.phase"  v-if="user.rol=='manager'" >
                    <option v-for="phase in phasesS" :key="phase.id" :value="phase.id">{{phase.name}}</option>
                </select>
            </div>
...
export default {
   ...
   project:String
    ...
    methods:{
        getPhases: function () {
          phaseService.getByProject(this.project).then(
            fases=>{
            console.log("fases");
            console.log(fases);
            this.haveData = true;
